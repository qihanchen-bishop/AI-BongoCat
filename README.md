...
`intervalSeconds` 控制心跳触发间隔，默认 60 秒。
...
这些文件用于保存长期记忆和定时任务，不会放在源码目录里。

## AI 输出格式

Gemini 会被要求输出结构化 JSON：
...
启用 AI 聊天后，你输入的内容以及被整理进提示词的长期记忆会发送给 Gemini API。请不要输入密码、私钥、API Key 或其他敏感信息。
