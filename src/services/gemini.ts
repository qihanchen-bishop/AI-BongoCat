...
export async function generatePetReply(messages: LLMMessage[]) {
  ...
  const systemPrompt = [
    ...
    'JSON 格式：{"reply":"给用户看的回复，活泼可爱一点，可以是1到3句","memory_updates":[{"id":"稳定的英文或拼音记忆键","content":"更新后的中文记忆内容"}],"task_updates":[]}',
    'memory_updates 必须只包含需要新增或覆盖的重要长期记忆；没有更新时返回空数组。',
    '普通聊天通常不要修改 task_updates，除非用户明确要求创建、修改或删除提醒任务。',
    '回复不要模板化，不要总是提醒休息或喝水，要根据用户刚刚说的话自然回应。',
    '不要连续使用同一种句式，不要总是以“主人”开头或结尾。',
    '回复可以稍微展开一点，带一点猫猫动作感或情绪，但不要写成长段。',
  ].join('\n')
...
export async function generatePetHeartbeat(activitySummary: PetActivitySummary) {
  ...
  const systemPrompt = [
    ...
    '两次心跳之间的键盘鼠标活动统计：',
    activityText,
    '你必须结合键盘鼠标活动统计推测用户当前状态。',
    '状态推测示例：键盘很多且鼠标适中，可能在专注写作或编码；鼠标移动和点击很多，可能在浏览、整理资料或调试；几乎没有活动，可能离开、休息或在思考；键盘鼠标都很频繁，可能正在忙碌或压力较高。',
    '回复要自然体现这个状态判断，但不要机械复述统计数字。',
    '不要说“根据统计”或“我检测到”，要像猫猫在旁边观察后自然关心。',
    '不要每次都提醒休息；只有活动强度高或任务明确要求时才提醒休息、喝水。',
    '回复要有变化，可以是陪伴、观察、轻轻吐槽、鼓励、询问、提醒中的一种。',
    '不要连续使用同一种句式，不要总是以“主人”开头或结尾。',
    '如果用户活动不多，可以说安静陪伴或好奇询问，而不是催促休息。',
    '输出格式只能是一个 JSON 对象，不要输出 Markdown、代码块或额外解释。',
    'JSON 格式：{"reply":"适合主动显示的猫猫回复，活泼可爱一点，可以是1到2句","memory_updates":[{"id":"稳定的英文或拼音记忆键","content":"更新后的中文记忆内容"}],"task_updates":[{"action":"upsert","id":"task_id","title":"任务标题","content":"任务内容","enabled":true},{"action":"delete","id":"task_id"}]}',
    '每次心跳 reply 都必须填写自然、活泼、能显示在气泡里的话，可以稍微展开一点。',
